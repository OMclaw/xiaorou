...
AEVIA_TARGET="${AEVIA_TARGET:-user:ou_0668d1ec503978ef15adadd736f34c46}"
...
if echo "$USER_INPUT" | grep -qiE "(发语音 | 语音消息 | 说句话 | 语音回复|voice|tts)"; then
  ...
  if /home/linuxbrew/.linuxbrew/bin/python3.9 "$SCRIPT_DIR/tts.py" "$SPEECH_TEXT" "$TEMP_AUDIO" 2>&1; then
    ...
    # 发送语音到频道
    # 支持通过 TARGET 参数或环境变量配置 target（默认 user: 前缀用于飞书私聊）
    if [ -n "$TARGET" ]; then
      SEND_TARGET="$TARGET"
    elif [ -n "${AEVIA_TARGET:-}" ]; then
      SEND_TARGET="${AEVIA_TARGET}"
    else
      SEND_TARGET="user:ou_0668d1ec503978ef15adadd736f34c46"
    fi
    ...
    if [ "$CHANNEL" = "feishu" ]; then
      ...
      # 调用 li-feishu-audio 技能发送语音气泡（需要配置 FEISHU_APP_ID 和 FEISHU_APP_SECRET）
      # 可通过环境变量或 openclaw.json 自动读取
