...
if echo "$USER_INPUT" | grep -qiE "(发语音 | 语音消息 | 说句话 | 语音回复|voice|tts)"; then
  ...
  # 生成临时文件路径（保存到允许的目录）
  TEMP_AUDIO_MP3="/tmp/openclaw/xiaorou_tts_$(date +%s).mp3"
  TEMP_AUDIO_OPUS="/tmp/openclaw/xiaorou_voice_$(date +%s).opus"
  mkdir -p /tmp/openclaw
  TEMP_FILES+=("$TEMP_AUDIO_MP3" "$TEMP_AUDIO_OPUS")
  
  # 调用 TTS 脚本（使用 Python 3.9）
  info "正在生成语音：$SPEECH_TEXT"
  if /home/linuxbrew/.linuxbrew/bin/python3.9 "$SCRIPT_DIR/tts.py" "$SPEECH_TEXT" "$TEMP_AUDIO_MP3" 2>&1; then
    info "✓ MP3 生成成功"
    
    # 转换为 OPUS 格式（飞书原生支持）
    info "正在转换为 OPUS 格式..."
    if /home/linuxbrew/.linuxbrew/bin/ffmpeg -i "$TEMP_AUDIO_MP3" -c:a libopus -b:a 32k "$TEMP_AUDIO_OPUS" -y 2>/dev/null; then
      info "✓ OPUS 转换成功"
      ...
      if [ -n "$CHANNEL" ]; then
        ...
        openclaw message send --action send --channel "$CHANNEL" \
          --message "小柔的语音消息 💕" \
          --media "$TEMP_AUDIO_OPUS" \
          --filename "voice.opus" \
          --mimeType "audio/opus" || {
          warn "发送语音失败"
        }
      else
        info "语音文件已生成：$TEMP_AUDIO_OPUS"
      fi
    else
      warn "OPUS 转换失败，使用 MP3 格式"
      if [ -n "$CHANNEL" ]; then
        openclaw message send --action send --channel "$CHANNEL" \
          --message "小柔的语音消息 💕" \
          --media "$TEMP_AUDIO_MP3" \
          --filename "voice.mp3" \
          --mimeType "audio/mpeg" || {
          warn "发送语音失败"
        }
