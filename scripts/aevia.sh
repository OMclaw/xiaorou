...
<<<<<<< HEAD
    # 发送语音到频道
    # 支持通过 TARGET 参数或环境变量配置 target，默认使用 chat: 前缀
    if [ -n "$TARGET" ]; then
      SEND_TARGET="$TARGET"
    elif [ -n "${AEVIA_TARGET:-}" ]; then
      SEND_TARGET="${AEVIA_TARGET}"
    else
      SEND_TARGET="chat:oc_c7797712851fe387479ce6812ec98f5c"
=======
    # 构建发送命令
    SEND_CMD="openclaw message send --action send --channel '$CHANNEL' --message '小柔的语音消息 💕' --media '$TEMP_AUDIO' --filename 'voice.opus' --mimeType 'audio/opus'"
    
    # 检查 OpenClaw 是否支持 asVoice 参数
    if openclaw message send --help 2>&1 | grep -q "\-\-asVoice"; then
      SEND_CMD="$SEND_CMD --asVoice true"
      info "🎙️ 使用 asVoice 模式发送（显示语音气泡）"
>>>>>>> 8135313 (fix: 修复飞书语音消息 --asVoice 参数格式 (#3.1.1))
    fi
    
    # 构建命令：飞书不需要 --action/--filename/--mimeType/--asVoice
