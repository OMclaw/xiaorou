...
def generate_from_reference(reference_image_path: str, caption: str = "这是模仿参考图生成的～", channel: Optional[str] = None, target: Optional[str] = None, multi_mode: bool = False) -> bool:
    ...
    4. 双模型并发生成（wan2.6-image + qwen-image-2.0-pro）
...
def generate_face_swap(target_image_path: str, caption: str = "换脸完成～看看效果怎么样？", channel: Optional[str] = None, target: Optional[str] = None) -> bool:
    ...
    **核心**：所有 4 张图都是小柔的脸，只是场景和风格不同
    
    方案：
    1. 方案一（natural）：自然融合 - 小柔自然融入目标图场景，场景可能微调更协调
    2. 方案二（precise）：精准换脸 - 小柔的脸 + 目标图场景精确还原，场景不变
    
    每个方案用两个模型各生成一张 = 4 张图
    **两个方案都用小柔头像作为输入图**（图生图模式）
    ...
    try:
        ...
        # 4. 生成两种方案的 prompt
        # 方案一：自然融合 - 小柔自然融入场景，整体协调柔和
        prompt_natural = f"""【自然融合模式】小柔进入这个场景，自然协调

**输入说明**：
- 输入图片是小柔的头像（只有脸部）
- 目标场景描述是从参考图中提取的场景、姿势、服装、光线等信息
- **参考图不包含脸部信息**，只学习场景

你是小柔，现在要进入这个场景中拍摄照片。
要求：
1. **必须是小柔本人**：使用输入图片中小柔的五官特征、脸型、发型
2. **自然融入场景**：学习目标场景描述的姿势、服装、光线氛围
3. **整体协调**：小柔与场景自然融合，光影协调，色调柔和
4. **场景可微调**：为了让小柔更自然融入，场景可以适当调整优化
5. 妆容清淡自然，裸妆效果，无腮红，无口红，裸唇，唇色自然
6. 真实摄影风格，自然光滑皮肤，清透肌肤，无 AI 感，无塑料感

目标场景描述：{scene_description}

请将小柔自然融入这个场景中，生成一张协调自然的照片。"""
        
        # 方案二：精准换脸 - 小柔的脸 + 场景精确还原
        prompt_precise = f"""【精准换脸模式】小柔的脸 + 场景精确还原
...
你是小柔，现在要在这个场景中拍摄照片，场景必须精确还原。
要求：
1. **严格保持小柔脸部特征**：使用输入图片中小柔的眼睛、鼻子、嘴巴、眉毛、脸型 100% 还原
2. **场景精确还原**：完全学习目标场景描述的姿势、服装、光线、构图
3. **场景不变**：只替换人物为小柔，场景保持原样，不做调整
4. **小柔特征明显**：确保是小柔本人，可识别度高
5. 妆容清淡自然，裸妆效果，无腮红，无口红，裸唇，唇色自然
6. 真实摄影风格，自然光滑皮肤，清透肌肤，无 AI 感

目标场景描述：{scene_description}

请将小柔放到这个场景中，严格保持场景不变，只替换人物为小柔。"""
        
        # 5. 生成 4 张图片（2 方案 × 2 模型）- 两个方案都用小柔头像作为输入
        models = ['wan2.6-image', 'qwen-image-2.0-pro']
        results = []
        
        # 方案一：自然融合 - 用小柔头像作为输入，图生图
        logger.info("🎨 方案一（自然融合）生成中...")
        for model_name in models:
            logger.info(f"  使用模型：{model_name}")
            model_result = generate_single_image(model_name, image_path, prompt_natural, api_key)
            if model_result[1]:
                results.append(('natural', model_name, model_result[1]))
        
        # 方案二：精准换脸 - 用小柔头像作为输入，图生图
        ...
        for scheme, model_name, image_url in results:
            if channel and image_url:
                ...
                # 根据方案选择配文
                if scheme == 'natural':
                    scheme_caption = "【自然融合】小柔自然融入场景，整体协调柔和"
                else:
