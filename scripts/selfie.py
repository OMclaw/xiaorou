...
def generate_face_swap(target_image_path: str, caption: str = "换脸完成～看看效果怎么样？", channel: Optional[str] = None, target: Optional[str] = None) -> bool:
    ...
    try:
        ...
        # 4. 生成两种方案的 prompt - 加大差异度
        # 方案一：自然融合（轻度换脸）- 保持原图人物 80% 相似度，只微调
        prompt_natural = f"""【自然融合模式 - 微调优化】
对输入图片进行轻度美化优化，保持原图人物的 80% 特征。
要求：
1. **保持原图人物的脸部特征为主**，只做轻微优化
2. 皮肤更光滑细腻，但保持原脸型
3. 五官微调：眼睛更有神，嘴唇更红润，但保持原样
4. 妆容清淡自然，裸妆效果，无腮红
5. 保持原图的场景、姿势、服装、发型、光线完全不变
6. 整体效果自然，像原图人物的精修版

原图描述：{scene_description}"""
        
        # 方案二：精准换脸（完全替换）- 用小柔头像进行图生图
        prompt_precise = f"""【精准换脸模式 - 完全替换】
使用输入的小柔头像，完全替换图中人物的脸部。
要求：
1. **必须使用小柔头像的五官特征**：小柔的眼睛、鼻子、嘴巴、眉毛、脸型
2. 保持小柔头像的身份特征，确保是小柔本人
3. 只保留原图的场景、姿势、服装、光线
4. 小柔的脸部特征要 100% 还原，不要融合原图人物
5. 妆容清淡自然，裸妆效果，无腮红，无口红，裸唇，唇色自然
6. 真实摄影风格，自然光滑皮肤，清透肌肤

这是小柔的头像（作为输入图片），请将这张脸放到原图的场景中，保持小柔的脸部特征完全不变。

原图场景描述：{scene_description}"""
        
        # 5. 生成 4 张图片（2 方案 × 2 模型）
        models = ['wan2.6-image', 'qwen-image-2.0-pro']
        results = []
        
        # 方案一：自然融合 - 用目标图作为输入，轻度美化
