...
def send_to_channel(image_url: str, caption: str, channel: str, model_name: str, target: Optional[str] = None) -> bool:
    ...
    Args:
        image_url: 图片 URL
        caption: 配文
        channel: 频道（默认 feishu）
        model_name: 模型名称
        target: 目标用户 ID（可选，为空时使用 openclaw 默认路由）
    """
    try:
        # 默认频道为 feishu
        if not channel:
        ...
        # 解析 target：参数 > 环境变量 > 配置文件 > auto-routing
        send_target = target or os.environ.get('AEVIA_TARGET', '')
        if not send_target and channel == 'feishu':
            send_target = config.get_feishu_target()
        
        # 飞书使用原生图片消息
        if channel == 'feishu':
            if send_target:
                # 有 target 时使用飞书原生 API
                ...
                if image_key:
                    if send_feishu_image_message(image_key, caption, send_target):
                        ...
                        logger.warning("⚠️ 飞书 API 发送失败，切换到 openclaw CLI")
                else:
                    logger.warning("⚠️ 图片上传失败，切换到 openclaw CLI")
            else:
                logger.info("ℹ️  未指定 target，使用 openclaw CLI 自动路由")
            
            # Fallback: 使用 openclaw CLI（支持 auto-routing）
            return _send_via_openclaw_cli(temp_file, caption, channel, send_target)
        else:
            # 其他平台使用 openclaw CLI
            return _send_via_openclaw_cli(temp_file, caption, channel, send_target)
    except Exception as e:
...
def _send_via_openclaw_cli(file_path: str, caption: str, channel: str, target: Optional[str] = None) -> bool:
    ...
    Args:
        ...
        caption: 配文
        channel: 频道
        target: 目标用户（可选，为空时自动路由到当前会话）
    """
    try:
        if target:
            cmd_args = ['openclaw', 'message', 'send', '--channel', channel, '--target', target, '--message', caption, '--media', file_path]
        else:
            # 没有 target 时使用 auto-routing（当前会话）
            cmd_args = ['openclaw', 'message', 'send', '--channel', channel, '--message', caption, '--media', file_path]
        
        logger.info(f"📤 openclaw CLI: {' '.join(cmd_args)}")
        result = subprocess.run(cmd_args, capture_output=True, text=True, timeout=60)
        if result.returncode == 0:
            logger.info("✓ openclaw CLI 发送成功")
            return True
        else:
            logger.error(f"❌ openclaw CLI 失败：{result.stderr}")
            return False
    except Exception as e:
        logger.error(f"openclaw CLI 异常:{e}")
        return False
...
def generate_edit_image(source_image_path: Path, edit_instruction: str, caption: str = "编辑完成~", channel: Optional[str] = None, target: Optional[str] = None) -> bool:
    ...
    try:
        ...
        # 发送生成的图片（默认 channel=feishu）
        effective_channel = channel or 'feishu'
        effective_target = target or os.environ.get('AEVIA_TARGET', '')
        if send_to_channel(image_url, caption, effective_channel, model_name, effective_target):
            logger.info("发送成功")
            return True
        else:
            logger.error("发送失败，请检查配置或查看上方错误日志")
            return False
