...
图 2（小柔头像）：提供人物身份和五官特征参考 -> **自然融合到整体画面中**
输出 = **一张全新的、自然融合的照片**，小柔完全融入场景，无拼贴感/无贴上去的感觉
...
【必须保持 - 图 2 内容】
- 小柔面部五官、发型发色
- **肤色智能适配**：保留小柔五官特征，但肤色亮度根据图 1 身体肤色自适应调整
- 五官比例关系不变形，仅做角度适配
...
权重 5.0：面部角度与原图不一致、面部与身体光影不一致、面部与身体色温不匹配、面部与身体肤色断层、头身比例失调、**头部过大/过小**、**脸部比例与原图不一致**、塑料皮肤、蜡质感、过度磨皮、娃娃脸、视线方向错误、**面部过白/惨白/死白**、**拼贴感/合成感/贴上去的感觉**
...
def generate_scene_image(scene_description: str, caption: str = "给你看看我现在的样子~", channel: Optional[str] = None, target: Optional[str] = None) -> bool:
    ...
    try:
        ...
        # 单图输入生成(仅小柔头像 + 文字 prompt)
        logger.info("🚀 wan2.7-image 生成中 (单图输入:小柔头像 + 场景描述)...")
...
def generate_scene_image_with_prompt(character_image_path: Path, prompt: str, api_key: str, max_retries: int = 2) -> Tuple[str, Optional[str]]:
    ...
    for attempt in range(max_retries + 1):
        try:
            character_base64 = get_image_base64(character_image_path)
            logger.info(f"🖼️ 单图输入模式:小柔头像 + 文字 prompt (尝试 {attempt + 1}/{max_retries + 1})")
            ...
            # 单图输入:小柔头像 + 文字 prompt
            content = [
                {'image': character_base64},      # 小柔头像
                {'text': prompt}                   # 场景描述
            ]
            logger.info(f"🖼️ 单图输入:小柔头像 + 场景描述 prompt")
