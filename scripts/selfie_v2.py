...
def build_role_swap_prompt(reference_description: str = "") -> str:
    ...
    # 完整 prompt - 格式化排版
    full_prompt = f"""【核心指令】
图 1：提取场景/服装/姿势/光影/构图 -> 人脸完全忽略
图 2：提取小柔的脸部/五官/发型/肤色 -> 100% 保留
输出 = 图 1 场景身体 + 图 2 脸部
...
【质量标签】8K 超高清，无水印，最高画质，杰作

【反向提示词】
权重 6.0：水印、文字、logo
权重 5.0：面部角度与原图不一致、面部与身体光影不一致、面部与身体色温不匹配、面部与身体肤色断层、头身比例失调、塑料皮肤、蜡质感、过度磨皮、娃娃脸、视线方向错误
权重 4.0：对比度异常、锐度不足、景深错误、透视失真、CG 渲染感、卡通化、油光满面、死白肤色、面部过亮/过暗、面部色偏、面部扭曲变形、低画质、畸形、解剖错误、过曝、欠曝、色偏"""
    return full_prompt
