...
支持模式:
1. 角色替换:参考图 + 图 2 → 小柔在参考图场景下 (保持服装/姿势/场景不变)

核心功能:
- 双图输入:第一张参考图 (场景/服装/姿势),第二张图 2 (人物身份)
...
图 2（人物正脸）：提供正面五官特征、脸型、发型 -> **人物身份主要参考**
图 3（人物多角度）：提供侧面轮廓、不同角度五官特征 -> **人物身份辅助参考**
输出 = **一张全新的、自然融合的照片**，综合图 2 和图 3 的特征，人物完全融入场景，无拼贴感/无贴上去的感觉
...
【必须保持 - 图 2/图 3 内容】
- 图 2（人物正脸）：提供正面五官特征、脸型、发型正脸
- 图 3（人物多角度）：提供侧面轮廓、不同角度五官特征、发型侧面
- **综合图 2 和图 3 的特征**：100% 保留图 2 人物身份，多角度参考确保一致性
- **肤色智能适配**：保留图 2 五官特征，但肤色亮度根据图 1 身体肤色自适应调整
- 五官比例关系不变形，仅做角度适配
...
【面部光影肤色匹配】
- **图 2 脸部的光源方向必须与参考图原人物脸部的光源方向完全一致**
- **图 2 脸部的肤色亮度必须与参考图原人物脸部的肤色亮度完全一致**
...
def generate_role_swap_image(reference_image_path: Path, character_image_path: Path, prompt: str, api_key: str, max_retries: int = 2) -> Tuple[str, Optional[str]]:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.info(f"🖼️ 双图输入模式:参考图 + 图 2 (尝试 {attempt + 1}/{max_retries + 1})")
            ...
            # 双图输入:参考图 (图 1) + 图 2 (图 2)
            # 图 1=参考图 (提供场景),图 2=小柔 (提供脸部)
            content = [
                {'image': reference_base64},      # 图 1: 参考图 (提供场景 - 要保留的场景)
                {'image': character_base64},      # 图 2: 图 2 (提供脸部 - 要替换的脸)
                {'text': prompt}                   # 角色替换提示词
            ]
            logger.info(f"🖼️ 双图输入:图 1(参考场景) + 图 2(图 2 脸部) + 文字 prompt")
...
def generate_role_swap_image_three(reference_image_path: Path, character1_path: Path, character2_path: Path, prompt: str, api_key: str, max_retries: int = 2) -> Tuple[str, Optional[str]]:
    ...
    Args:
        reference_image_path: 参考图路径 (场景/服装/姿势)
        character1_path: 图 2 1 路径 (正脸参考)
        character2_path: 图 2 2 路径 (多角度参考)
        prompt: 提示词
        api_key: API Key
        max_retries: 最大重试次数
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.info(f"🖼️ 三图输入模式:参考图 + 图 2 1 + 图 2 2 (尝试 {attempt + 1}/{max_retries + 1})")
            ...
            # 三图输入：参考图 (图 1) + 图 2 1(图 2) + 图 2 2(图 3)
            # 图 1=参考图 (提供场景), 图 2=图 2 (提供正面五官), 图 3=图 3 (提供侧面/不同角度特征)
            content = [
                {'image': reference_base64},      # 图 1: 参考图 (提供场景)
                {'image': character1_base64},     # 图 2: 图 2 1 (正脸参考)
                {'image': character2_base64},     # 图 3: 图 2 2 (多角度参考)
                {'text': prompt}                   # 角色替换提示词
            ]
...
def generate_scene_image(scene_description: str, caption: str = "给你看看我现在的样子~", channel: Optional[str] = None, target: Optional[str] = None) -> bool:
    """
    场景生图模式:纯文字描述 → 小柔在该场景下

    Args:
        scene_description: 场景描述(如"在海边看日落")
        caption: 发送消息的配文
        channel: 发送频道
        target: 发送目标
    ...
    try:
        ...
        # 构建场景生图 prompt
        prompt = f"网红风格,时尚穿搭,专业摄影,清淡妆容,{scene_description},自然真实,8K 超高清"
        logger.info(f"🏞️ 场景生图模式:{scene_description}")

        # 单图输入生成(仅图 2 + 文字 prompt)
        logger.info("🚀 wan2.7-image 生成中 (单图输入:图 2 + 场景描述)...")
        model_name, image_url = generate_scene_image_with_prompt(character_path, prompt, api_key)
        ...
        logger.error(f"场景生图错误:{e}")
        return False


def generate_scene_image_with_prompt(character_image_path: Path, prompt: str, api_key: str, max_retries: int = 2) -> Tuple[str, Optional[str]]:
    """
    使用 wan2.7-image 进行场景生图 (单图输入)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # 单图输入:图 2 + 文字 prompt
            content = [
                {'image': character_base64},      # 图 2
                {'text': prompt}                   # 场景描述
            ]
            logger.info(f"🖼️ 单图输入:图 2 + 场景描述 prompt")
...
def generate_role_swap(reference_image_path: str, caption: str = "这是角色替换生成的~", channel: Optional[str] = None, target: Optional[str] = None) -> bool:
    ...
    try:
        ...
        # 加载图 2 1（正脸参考）
        character_path = validate_character_image()
        logger.info("✅ 图 2 1 验证通过（正脸参考）")

        # 加载图 2 2（多角度参考）
        character_ref_2 = Path("/home/admin/.openclaw/workspace/skills/xiaorou/assets/character_ref_2.png")
        if character_ref_2.exists():
            logger.info("✅ 图 2 2 验证通过（多角度参考）")
            use_three_images = True
        else:
            logger.info("ℹ️ 未找到图 2 2，使用双图模式")
        ...
        # 三图/双图输入生成
        if use_three_images:
            logger.info("🚀 wan2.7-image 生成中 (三图输入:参考图 + 图 2 1 + 图 2 2)...")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  场景生图:python3 selfie_v2.py --scene <场景描述> [频道] [配文] [target]")
        sys.exit(1)
    ...
    if sys.argv[1] == '--role-swap' and len(sys.argv) >= 3:
        ...
        channel = sys.argv[3] if len(sys.argv) > 3 else None
        caption = sys.argv[4] if len(sys.argv) > 4 else "给你看看我现在的样子~"
        target = sys.argv[5] if len(sys.argv) > 5 else None
        # 场景生图:使用优化版 prompt,仅文字描述生成
        logger.info(f"🏞️ 场景生图模式:{scene_description}")
        prompt = build_structured_prompt(scene_description)
        # 调用 wan2.7-image 文生图
        success = generate_scene_image(scene_description, caption, channel, target)
        sys.exit(0 if success else 1)
